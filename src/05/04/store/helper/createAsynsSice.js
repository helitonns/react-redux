...
/**
 * Cria um slice com um função assícrona
 * @param {Object} config 
...
const createAsyncSlice = (config) => {
  const slice = createSlice({
    ...
    reducers:{
      ...
      updateTime(state, action){
        state.lastUpadate = action.payload;
      },
  ...
  const asyncAction = (payload) => async (dispatch, getState) => {
    const {lastUpadate, cahe: cache} = getState()[slice.name];
    console.log(lastUpadate);
    if(lastUpadate > Date.now() - cache) return;
    try {
